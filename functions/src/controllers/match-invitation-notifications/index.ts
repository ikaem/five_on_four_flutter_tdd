...
export const handleMatchInvitationNotifications = async (
  ...
  if (collectionsError !== undefined) {
    response.status(500).send('App has not been initialized');
    return;
  }
...
// // export const handleMatchInvitationNotifications =
// //   firebaseFunctionsWrapper.callFunction(async (request, response) => {
// //     // FUTURE abstract authetnication this if more functions added
// //     const { authorization } = request.headers;
// //     if (authorization === undefined) {
// //       response.status(404).send('No authorization header found');
