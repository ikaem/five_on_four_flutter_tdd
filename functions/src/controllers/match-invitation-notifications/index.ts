  ...
  try {
    ...
    if (authorization === undefined) {
      response.status(404).json({
        data: { ok: false, message: 'No authorization header found' },
      });
      return;
    }

    const isAuthHeaderValidFormat: boolean =
    ...
    if (authError !== undefined) {
      response
        .status(401)
        .json({ data: { ok: false, message: 'Unauthorized' } });
      return;
    }
    ...
    if (validationError !== undefined) {
      response
        .status(400)
        .json({ data: { message: 'Invalid body', ok: false } });
      return;
    }
    ...
    console.log(matchInvitationFullMessages);
